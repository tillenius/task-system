	...
	for (size_t j = 0; j < numBlocks; j++) {
		for (size_t k = 0; k < j; k++) {
			for (size_t i = j + 1; i < numBlocks; i++) {
				...
				ts.submit2(ts.shared, {&h[i * numBlocks + k], &h[j * numBlocks + k]}, {&h[i * numBlocks + j]}, [i, j, k](Context) {
		...
		for (size_t i = 0; i < j; i++) {
			...
			ts.submit2(ts.shared, {&h[j * numBlocks + i] }, {&h[j * numBlocks + j]}, [i, j](Context) {
		...
		ts.submit2(ts.main, {}, {&h[j * numBlocks + j]}, [j](Context) {
		...
		for (size_t i = j + 1; i < numBlocks; i++) {
			...
			ts.submit2(ts.shared, {&h[j * numBlocks + j]}, {&h[i * numBlocks + j]}, [i, j](Context) {
